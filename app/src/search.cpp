		...
		while(ultimate_max_depth == -1 || max_depth <= ultimate_max_depth) {
			...
			if (sp.stop->flag) {
				...
				if (sp.is_t2 == false && output)
					printf("info string stop flag set\n");
#endif
				if (sp.is_t2 == false && output)
			...
			if (score <= alpha) {
...
#if !defined(__ANDROID__)
					if (output)
						printf("info string time %u is up %" PRIu64 "\n", search_time, thought_ms);
#endif
					break;
				}
				...
				if (max_n_nodes.has_value() && cur_n_nodes >= max_n_nodes.value()) {
					if (output)
						printf("info string node limit reached with %zu nodes\n", size_t(cur_n_nodes));
					break;
				}
...
#if !defined(__ANDROID__)
		if (output)
			printf("info string only 1 move possible (%s for %s)\n", best_move.to_str().c_str(), pos.fen().c_str());
#endif
		best_score = eval(pos, sp.parameters);
	}
