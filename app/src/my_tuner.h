...
class NormalizedResult {
   public:
    ...
    [[nodiscard]] int get_result() const noexcept {
        return result_;
    }
    void set_result(const int v) noexcept {
        result_ = v;
    }
...
class Tuner {
   public:
    Tuner(std::vector<NormalizedResult<Position>> normalized_results,
          ...
          std::function<void(std::vector<NormalizedResult<Position>> &, const std::vector<TunableParameter>&)> eval_function)
...
#pragma omp parallel for reduction(+ : sum)
    ...
    void local_tune() noexcept {
		...
		finished += tuning_data.done();
            }
            std::cout << "Least error: " << least_error << ", finished: " << finished << "\n";
   ...
   protected:
    ...
    void eval(std::vector<NormalizedResult<Position> > & positions) noexcept {
        eval_function_(positions, tunable_parameters_);
    }
   ...
   private:
    ...
    std::function<void(std::vector<NormalizedResult<Position> > & positions, const std::vector<TunableParameter>&)> eval_function_{};
};
